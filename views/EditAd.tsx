...
export const EditAd: React.FC<EditAdProps> = ({ adId }) => {
  ...
  return (
    <div className="px-5 pt-8 pb-20">
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.values(ServiceCategory).map(cat => (
              <div 
                key={cat} 
                className={`bg-surface border rounded-xl p-3 text-center text-sm font-medium cursor-pointer transition-colors ${
                  worker.category === cat 
        ...
        <div className="pt-4">
          <Button fullWidth onClick={() => router.back()}>{t('save_changes')}</Button>
        </div>
      </div>
    </div>
  );
